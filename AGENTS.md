...
- **Rebuild TextMate grammars after editing YAML sources.** After modifying any `syntaxes/*.tmLanguage.yml` file, run `scripts/syntaxes-to-json.sh` to regenerate the compiled JSON before testing or committing.
- **Do not hand-edit auto-generated TextMate stanzas.** 19 stanzas in `syntaxes/weidu-tp2.tmLanguage.yml` are generated from `server/data/weidu-tp2-base.yml` by `scripts/utils/src/update-tp2-highlight.ts`; 8 stanzas in `syntaxes/weidu-d.tmLanguage.yml` are generated from `server/data/weidu-d-base.yml` by `scripts/utils/src/update-d-highlight.ts`; 2 stanzas (`actions`, `triggers`) in `syntaxes/weidu-baf.tmLanguage.yml` are generated from `server/data/weidu-baf-iesdp.yml` by `scripts/utils/src/update-baf-highlight.ts`. All are called via `generate-data.sh`. Edit the YAML data source, then regenerate. Auto-generated stanzas are marked with a `# Auto-generated` comment. See `docs/data-pipeline.md` for the full data flow.
- **Sort YAML data files with the existing script.** To sort `server/data/*.yml` files, use `pnpm exec tsx scripts/utils/src/sort-yaml-stanzas-and-items.ts <file>`. To sort `patterns` within all stanzas of a TextMate grammar (without reordering stanzas), use `--map-path repository --sequence-key patterns --sort-key match`. Do not write custom sorting code. See `scripts/README.md` for all available script utilities.
