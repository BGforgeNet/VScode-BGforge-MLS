...
ALTER_TRANS ~%tutu_var%FLAMEN~ // file name
BEGIN 0 4 END // state number (can be more than one)
BEGIN 0 1 END // transition number (can be more than one)
BEGIN // list of changes, see below for flags
  "TRIGGER" ~CheckStatLT(LastTalkedToBy(Myself),10,CHR) ReputationLT(LastTalkedToBy(Myself),10)~
END

EXTEND_TOP ~%tutu_var%FLAMEN~ 0
    IF ~CheckStatGT(LastTalkedToBy(Myself),9,CHR)~ THEN REPLY @9800 GOTO RR#00
    ...
    IF ~ReputationGT(LastTalkedToBy(Myself),9)~ THEN REPLY @9801 GOTO RR#01
    // \[Reputation\] - My sincere apologies, officer. As an upstanding citizen, I would prefer to pay the appropriate fine and settle this matter in a civilized manner.
END

EXTEND_TOP ~%tutu_var%FLAMEN~ 4
    IF ~CheckStatGT(LastTalkedToBy(Myself),9,CHR)~ THEN REPLY @9800 GOTO RR#00
    // \[Charisma\] - This is simply a misunderstanding, I assure you. I will gladly pay the appropriate fine and resolve this issue peacefully.
    IF ~ReputationGT(LastTalkedToBy(Myself),9)~ THEN REPLY @9801 GOTO RR#01
    // \[Reputation\] - My sincere apologies, officer. As an upstanding citizen, I would prefer to pay the appropriate fine and settle this matter in a civilized manner.
