...
// scripts/ie-update/src/ielib-update.ts
...
var COMPLETION_TYPE_FUNCTION = 3;
  ...
  function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const fileExt = import_node_path.default.extname(entry.name).toLowerCase();
...
function dumpCompletion(fpath, iedata) {
  ...
  import_node_fs.default.writeFileSync(fpath, output, "utf8");
}
...
function dumpHighlight(fpath, iedata) {
  const content = import_node_fs.default.readFileSync(fpath, "utf8");
...
// scripts/ie-update/src/ielib-update.ts
var IELIB_URL = "https://ielib.bgforge.net";
var TYPES_URL = `${IELIB_URL}/types`;
var REGEX_NUMERIC = /^(?:OUTER_SET\s+)?(\w+)\s*=\s*(\w+)/;
var REGEX_TEXT = /^(?:OUTER_SPRINT|TEXT_SPRINT)\s+~?(\w+)~?\s+~(\w+)~/;
function definesFromFile(filePath, regex) {
...
function getDefault(param, func) {
  if (param.default !== void 0) {
    return param.default;
  }
  if (func.defaults !== void 0 && func.defaults[param.type] !== void 0) {
    return func.defaults[param.type];
  }
  return "";
}
function paramsToMd(func, ptype, types) {
  const typeMap = {
...
|:-|:-|:-|:-|`;
  const rows = sortedParams.map((param) => {
    const defaultVal = param.required === 1 ? "_required_" : getDefault(param, func);
...
function retsToMd(func, types) {
  if (func.return === void 0) {
    return "";
  }
  const sortedRets = [...func.return].sort((a, b) => a.name.localeCompare(b.name));
...
function funcToItem(func, types) {
  let text = `${func.desc}

`;
  if (func.int_params !== void 0) {
    text += paramsToMd(func, "int_params", types);
  }
  if (func.string_params !== void 0) {
...
function main() {
  ...
  let intDefines = /* @__PURE__ */ new Map();
  let resrefDefines = /* @__PURE__ */ new Map();
  for (const df of defineFiles) {
    const newIntDefines = definesFromFile(df, REGEX_NUMERIC);
    intDefines = new Map([...intDefines, ...newIntDefines]);
    const newResrefDefines = definesFromFile(df, REGEX_TEXT);
    resrefDefines = new Map([...resrefDefines, ...newResrefDefines]);
  }
  const intItems = [...intDefines.entries()].map(
    ([name, detail]) => ({
      name,
  ...
  const resrefItems = [...resrefDefines.entries()].map(
    ([name, detail]) => ({
      name,
  ...
  const dataDir = import_node_path2.default.join(srcDir, "docs", "_data");
  const functionDir = import_node_path2.default.join(dataDir, "functions");
  const functionFiles = findFiles(functionDir, "yml");
  const typesFile = import_node_path2.default.join(dataDir, "types.yml");
  const types = import_yaml3.default.parse(import_node_fs2.default.readFileSync(typesFile, "utf8"));
  const actionFunctions = [];
  const patchFunctions = [];
  for (const f of functionFiles) {
    const data = import_yaml3.default.parse(import_node_fs2.default.readFileSync(f, "utf8"));
...
//# sourceMappingURL=ielib-update.js.map
