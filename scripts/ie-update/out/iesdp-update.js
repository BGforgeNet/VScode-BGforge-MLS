...
// scripts/ie-update/src/ie/opcodes.ts
var REPLACEMENTS = [
  [" ", "_"],
  [")", "_"],
  ["(", "_"],
...
var LEFT_STRIP = [
  ...
  "graphics_",
  "spell_effect_",
  // must be before spell_
  "spell_",
  "stat_",
  "state_",
  "summon_"
];
function opcodeNameToId(name) {
...
// scripts/ie-update/src/ie/offsets.ts
var SIZE_MAP = {
  byte: 1,
  char: 1,
  word: 2,
...
function getOffsetPrefix(fileVersion, dataFileName) {
...
function getFormatVersion(fileVersion) {
  const base = fileVersion.replace(/_v.*/, "");
  let version = fileVersion.replace(/.*_v/, "");
  version = version.replaceAll(".", "");
  if (version === "1") {
...
function getOffsetId(item, prefix) {
  if (item.id !== void 0) {
    return prefix + item.id;
  }
  return stringToId(item.desc, prefix);
}
function stringToId(line, prefix) {
...
function validateOffset(currentOffset, item) {
  if (item.offset !== void 0 && item.offset !== currentOffset) {
    throw new Error(
      `Offset mismatch. Expected ${currentOffset}, got ${item.offset} for ${JSON.stringify(item)}`
    );
  }
}
function offsetIsUnused(offset) {
  if (offset.unused !== void 0 || offset.unknown !== void 0) {
    return true;
  }
  if (offset.desc !== void 0 && offset.desc.toLowerCase() === "unknown") {
    return true;
  }
  return false;
}
function offsetsToDefinition(data, prefix) {
...
// scripts/ie-update/src/ie/item-types.ts
var import_node_fs = __toESM(require("node:fs"));
var import_node_path = __toESM(require("node:path"));
var import_yaml = __toESM(require_dist());
var ITEM_TYPE_PREFIX = "ITEM_TYPE_";
function getItemTypes(iesdpFileFormatsDir) {
  const sourceFile = import_node_path.default.join(iesdpFileFormatsDir, "item_types.yml");
  const content = import_node_fs.default.readFileSync(sourceFile, "utf8");
  const items = import_yaml.default.parse(content);
...
function saveItemTypesIelib(ielibStructuresDir, itypes) {
  const destFile = import_node_path.default.join(ielibStructuresDir, "item_types.tpp");
  const text = itypes.map((itype) => `${itype.id} = ${itype.value}`).join("\n") + "\n";
  import_node_fs.default.writeFileSync(destFile, text + "\n", "utf8");
}
function getItemTypesIsense(itypes) {
...
// scripts/ie-update/src/ie/actions.ts
function actionAliasDesc(actions, action) {
  const num = action.alias === true ? action.n : action.alias;
  const parent = actions.find((x) => x.n === num && x.alias === void 0);
  if (parent === void 0) {
    return false;
  }
  if (parent.unknown !== void 0) {
    return false;
  }
  return parent.desc ?? false;
}
function actionDesc(actions, action, iesdpGames, iesdpBaseUrl) {
  let desc;
  if (action.alias !== void 0) {
  ...
  const gameName = action.bg2 !== void 0 ? "bg2" : "bgee";
  return actionDescAbsoluteUrls(desc, iesdpGames, gameName, iesdpBaseUrl);
}
function actionDescAbsoluteUrls(desc, games, gameName, iesdpBaseUrl) {
...
function appendUnique(actions, newActions) {
...
// scripts/ie-update/src/ie/common.ts
var import_node_fs2 = __toESM(require("node:fs"));
var import_node_path2 = __toESM(require("node:path"));
var import_yaml2 = __toESM(require_dist());

// scripts/ie-update/src/ie/types.ts
var COMPLETION_TYPE_CONSTANT = 21;

// scripts/ie-update/src/ie/common.ts
var YAML_DUMP_OPTIONS = {
  lineWidth: 4096,
  indent: 2,
  indentSeq: true
};
function litscal(text) {
...
function findFiles(dirPath, ext, skipDirs = [], skipFiles = ["iesdp.tph"]) {
  const results = [];
  ...
  function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const fileExt = import_node_path2.default.extname(entry.name).toLowerCase();
...
function sortLongerFirst(a, b) {
  if (a.length !== b.length) {
    return b.length - a.length;
  }
  return a < b ? -1 : a > b ? 1 : 0;
}
function makeBlockScalar(doc, value) {
  const scalar = doc.createNode(value);
  scalar.type = import_yaml2.Scalar.BLOCK_LITERAL;
  return scalar;
}
function createItemsSeq(doc, items, forceBlockDoc = false) {
  const seq = new import_yaml2.YAMLSeq();
  for (const item of items) {
    const map = new import_yaml2.YAMLMap();
    map.add(doc.createPair("name", item.name));
    map.add(doc.createPair("detail", item.detail));
    const docValue = forceBlockDoc || item.doc.includes("\n") ? makeBlockScalar(doc, item.doc) : item.doc;
...
function dumpHighlight(fpath, iedata) {
  const content = import_node_fs2.default.readFileSync(fpath, "utf8");
  const doc = import_yaml2.default.parseDocument(content);
  const repository = doc.getIn(["repository"], true);
  if (!(0, import_yaml2.isMap)(repository)) {
  ...
  for (const [, ied] of Object.entries(iedata)) {
    ...
    const patternsSeq = doc.createNode(isStringCategory ? [...sortedNames.map((n) => ({ match: `(%${n}%)` })), ...wordPatterns] : wordPatterns);
    if ((0, import_yaml2.isMap)(stanzaNode)) {
...
function dumpDefinition(prefix, items, structuresDir) {
  const outputDir = import_node_path2.default.join(structuresDir, prefix.toLowerCase().replaceAll("_", ""));
  const outputFile = import_node_path2.default.join(outputDir, "iesdp.tpp");
  import_node_fs2.default.mkdirSync(outputDir, { recursive: true });
  const sortedEntries = [...items.entries()].sort(([a], [b]) => a.localeCompare(b));
  const text = sortedEntries.map(([name, value]) => `${name} = ${value}`).join("\n") + "\n";
  import_node_fs2.default.writeFileSync(outputFile, text + "\n", "utf8");
}
function stripLiquid(text) {
  return text.replace(/{% capture note %}/g, "").replace(/{% endcapture %} {% include note\.html %}/g, "").replace(/{% endcapture %} {% include info\.html %}/g, "");
}

// scripts/ie-update/src/iesdp-update.ts
var SKIP_OPCODE_NAMES = ["empty", "crash", "unknown"];
var IESDP_BASE_URL = "https://gibberlings3.github.io/iesdp/";
var ACTIONS_STANZA = "actions";
function parseFrontmatter(filePath) {
  ...
  return import_yaml3.default.parse(fmMatch[1]);
}
function appendOffsets(pod, offsetData, offsetPrefix) {
  ...
  for (const item of offsetData) {
    ...
    const hasMult = item.mult !== void 0;
    const detail = hasMult ? `multi ${item.type} offset ${iid} = ${itemOff}` : `${item.type} offset ${iid} = ${itemOff}`;
...
function main() {
  ...
  const iesdpFileFormatsDir = import_node_path3.default.join(iesdpDir, "_data", "file_formats");
  const ielibStructuresDir = import_node_path3.default.join(ielibDir, "structures");
  const opcodeFile = import_node_path3.default.join(ielibDir, "misc", "opcode.tpp");
  const opcodeDir = import_node_path3.default.join(iesdpDir, "_opcodes");
  const opcodeFiles = findFiles(opcodeDir, "html");
  const rawOpcodes = opcodeFiles.map(parseFrontmatter).filter((o) => o["bg2"] === 1).sort((a, b) => a["n"] - b["n"]);
  const opcodesUnique = /* @__PURE__ */ new Map();
...
`;
  ...
  import_node_fs3.default.writeFileSync(opcodeFile, tppText + "\n", "utf8");
  const itemTypes = getItemTypes(iesdpFileFormatsDir);
  saveItemTypesIelib(ielibStructuresDir, itemTypes);
  const actionsDir = import_node_path3.default.join(iesdpDir, "_data", "actions");
  const actionFiles = findFiles(actionsDir, "yml");
  const iesdpGamesFile = import_node_path3.default.join(iesdpDir, "_data", "games.yml");
  const iesdpGames = import_yaml3.default.parse(
    import_node_fs3.default.readFileSync(iesdpGamesFile, "utf8")
  );
  let actions = [];
  for (const f of actionFiles) {
    const action = import_yaml3.default.parse(import_node_fs3.default.readFileSync(f, "utf8"));
    if (action.bg2 !== void 0 && action.bg2 === 1 || action.bgee !== void 0 && action.bgee === 1) {
      actions.push(action);
    }
  }
  actions = actions.sort((a, b) => a.n - b.n);
  const actionsHighlight = [...new Set(actions.map((x) => x.name))];
  const actionsHighlightPatterns = actionsHighlight.map((x) => ({ match: `\\b(${x})\\b` })).sort((a, b) => a.match.localeCompare(b.match));
  const bafHighlightContent = import_node_fs3.default.readFileSync(highlightBaf, "utf8");
  const bafHighlightDoc = import_yaml3.default.parseDocument(bafHighlightContent);
  const bafRepo = bafHighlightDoc.getIn(["repository"], true);
  if (bafRepo && typeof bafRepo === "object" && "set" in bafRepo) {
    bafRepo.set(
      ACTIONS_STANZA,
      { patterns: actionsHighlightPatterns }
    );
    const stanzaNode = bafHighlightDoc.getIn(["repository", ACTIONS_STANZA], true);
    if (stanzaNode && typeof stanzaNode === "object" && "set" in stanzaNode) {
      stanzaNode.set(
        "patterns",
  ...
  const parentsBg2 = actions.filter((x) => x.bg2 !== void 0 && x.alias === void 0);
  const aliasesBg2 = actions.filter((x) => x.bg2 !== void 0 && x.alias !== void 0);
  const parentsBgee = actions.filter(
    (x) => x.bgee !== void 0 && x.bg2 === void 0 && x.alias === void 0
  );
  ...
  const sortedActionsCompletion = actionsCompletion.sort(
    (a, b) => a.name.localeCompare(b.name)
  );
  const bafDataContent = import_node_fs3.default.readFileSync(dataBaf, "utf8");
  const bafDataDoc = import_yaml3.default.parseDocument(bafDataContent);
  ...
  const formats = import_node_fs3.default.readdirSync(iesdpFileFormatsDir, { withFileTypes: true }).filter((e) => e.isDirectory()).map((e) => e.name);
  for (const ff of formats) {
    console.log(ff);
    const ffDir = import_node_path3.default.join(iesdpFileFormatsDir, ff);
    let definitionItems = /* @__PURE__ */ new Map();
    const files = import_node_fs3.default.readdirSync(ffDir).filter((f) => f !== "feature_block.yml");
    for (const f of files) {
      console.log(f);
      const prefix = getOffsetPrefix(ff, f);
      const fpath = import_node_path3.default.join(ffDir, f);
      const offsets = import_yaml3.default.parse(import_node_fs3.default.readFileSync(fpath, "utf8"));
  ...
  const featureBlockPath = import_node_path3.default.join(iesdpFileFormatsDir, "itm_v1", "feature_block.yml");
  const featureBlockOffsets = import_yaml3.default.parse(
    import_node_fs3.default.readFileSync(featureBlockPath, "utf8")
  );
  const PREFIX_FX = "FX_";
...
//# sourceMappingURL=iesdp-update.js.map
